...
func runTailCommand(ctx context.Context, repoRoot string, options TailOptions, stdout io.Writer, stderr io.Writer) error {
	...
	if options.Follow {
		return followLog(ctx, logPath, options.Lines, options.Raw, options.Output, stdout, stderr)
	}

	lines, err := readTailLines(logPath, options.Lines, options.Raw)
	if err != nil {
		return err
	}
	...
	default:
		...
		for _, line := range lines {
			_, _ = fmt.Fprintln(stdout, line.Line)
		}
		return nil
	}
}
...
func listRalphLogs(repoRoot string, selector string) ([]ralphLogFile, error) {
	...
	err := filepath.WalkDir(worktreesRoot, func(path string, entry fs.DirEntry, walkErr error) error {
		...
		log := ralphLogFile{Path: path, RelativePath: relativePath, MtimeMS: info.ModTime().UnixMilli()}
	...
	sort.Slice(logs, func(i, j int) bool { return logs[i].MtimeMS > logs[j].MtimeMS })
	return logs, nil
}

func readTailLines(path string, lines int, raw bool) ([]tailLineRecord, error) {
	...
	records := make([]tailLineRecord, 0, len(parts))
	for _, part := range parts {
		line := part
		if !raw {
			line = formatLiveLine(part)
		}
...
func followLog(ctx context.Context, path string, lines int, raw bool, output OutputFormat, stdout io.Writer, stderr io.Writer) error {
...
func formatLiveLine(line string) string {
	...
	if err := json.Unmarshal([]byte(envelope.Payload), &payload); err != nil {
		return strings.TrimSpace(prefix + " " + envelope.Payload)
	}
...
func parseLogEnvelope(line string) (logEnvelope, bool) {
	...
	return logEnvelope{Timestamp: parts[0], Channel: strings.TrimSuffix(parts[1], ":"), Payload: parts[2]}, true
}
...
func summarizeItemCompleted(item any) string {
	...
	switch valueString(record["type"]) {
	case "agentMessage":
		text := strings.TrimSpace(valueString(record["text"]))
		if text == "" {
			text = normalizeAgentMessage(record)
		}
		return text
	case "commandExecution":
